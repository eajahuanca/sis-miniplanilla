...
class PlanillazaireController extends Controller
{
    ...
    {
        return view('sistema.planilla.planillazaire.planillaIndex');
    }
    ...
    public function store(Request $request)
    {
        ...
        $pago = Asignar::join('empleado','empleado.id','=','asignar_empresa.id_empleado')
                ...
                ->orderBy('pago.created_at','ASC')->get();
                //->orderBy('empleado.em_fecha_ingeso','ASC')->get();
        
        $fechaImpresion = 'El Alto, '.date('d').' de '.$this->fecha().' de '.date('Y');

        $view = \View::make('sistema.planilla.planillazaire.planillaStore', compact('empresa','fechaImpresion','mes','gestion','tipo_empleado','pago'))->render();
