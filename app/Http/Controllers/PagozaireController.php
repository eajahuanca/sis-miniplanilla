...
class PagozaireController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('sistema.pago.pagozaire.pagoIndex')->with('pago', $pago);
    }
    ...
    public function create()
    {
        ...
        return view('sistema.pago.pagozaire.pagoCreate')->with('empleado',$empleado)->with('idsalariominimo', $idsalariominimo);
    }
    ...
    public function store(PagoRequest $request)
    {
        ...
        if(count($pagoVerificar)==0)
        {
            try
            {
                ...
                //$this->rcIva($totalGanado, $pago->pag_total_ganado);
        ...
        return redirect()->route('pagoZaire.index');
    }

    public function show($id)
    {
        $pago = Pago::find($id);
        return view('sistema.pago.pagozaire.pagoShow')->with('pago',$pago);
    }
    ...
    public function edit($id)
    {
        ...
        return view('sistema.pago.pagozaire.pagoEdit')->with('pago',$pago)->with('empleado',$empleado)->with('idsalariominimo',$idsalariominimo);
    }
    ...
    public function boletaPago($idPago)
    {
        ...
        $view = \View::make('sistema.pago.pagozaire.pagoReporte', compact('pago','empleado','empresa'))->render();
    ...
    public function bonoAntiguedad($idEmpleado, $idSalarioMinimo)
    {
        $empleado = Empleado::find($idEmpleado);
        $salarioMinimo = Salariominimo::find($idSalarioMinimo);
        $fechaIngreso = explode('-',$empleado->em_fecha_ingeso);
        $anosCalculados = Carbon::createFromDate((int)$fechaIngreso[0],(int)$fechaIngreso[2],(int)$fechaIngreso[1])->age;
        $porcentaje = 0;

        if($anosCalculados >= 0 && $anosCalculados < 2)
        {
            $porcentaje = 0;
        }
        if($anosCalculados >= 2 && $anosCalculados < 5)
        {
            $porcentaje = 5;
        }
        if($anosCalculados >= 5 && $anosCalculados < 8)
        {
            $porcentaje = 11;
        }
        if($anosCalculados >= 8 && $anosCalculados < 11)
        {
            $porcentaje = 18;
        }
        if($anosCalculados >= 11 && $anosCalculados < 15)
        {
            $porcentaje = 26;
        }
        if($anosCalculados >= 15 && $anosCalculados < 20)
        {
            $porcentaje = 34;
        }
        if($anosCalculados >= 20 && $anosCalculados < 25)
        {
            $porcentaje = 42;
        }
        if($anosCalculados >= 25)
        {
            $porcentaje = 50;
        }

        return ((3*$salarioMinimo->sm_salario)*($porcentaje/100));
    }
