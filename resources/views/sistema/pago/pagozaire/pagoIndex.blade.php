    ...
    <div class="form-group">
        <div class="row">
            <div class="col-xs-12 col-md-12">
                <span class="hint--top  hint--success" aria-label="Registrar Nuevo Pago de Zaire"><a  href="{{ route('pagoZaire.create') }}" class="btn btn-success"><i class="fa fa-plus"></i> Adicionar Pago Zaire</a></span>
            </div>
        </div>
    </div>
            ...
            <tr id="{{ $item->id }}">
                ...
                <td align="center">
                    <div class="form-horizontal">
                        @if($item->pag_estado)
                            <span class="hint--top  hint--error" aria-label="Obtener Boleta de Pago de: {{ $item->nombre_empleado }}"><a href="{{ url('/boletazr', $item->id) }}" class="btn btn-danger btn-xs"><i class="fa fa-file"></i></a></span>
                        @endif
                        <span class="hint--top  hint--info" aria-label="Actualizar Pago de: {{ $item->nombre_empleado }}"><a href="{{ route('pagoZaire.edit', $item->id) }}" class="btn btn-primary btn-xs"><i class="fa fa-edit"></i></a></span>
                        <span class="hint--top  hint--warning" aria-label="Ver Pago de: {{ $item->nombre_empleado }}"><a href="{{ route('pagoZaire.show', $item->id) }}" class="btn btn-warning btn-xs"><i class="fa fa-eye"></i></a></span>
                    </div>
                </td>
